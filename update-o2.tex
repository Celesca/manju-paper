...
\begin{enumerate}[label=(\roman*)]
    \item Build a complete voice-to-voice pipeline that responds in under five seconds.
    \item Provide a drag-and-drop workflow editor requiring no programming knowledge.
    \item Ground every answer in real documents or live data to reduce AI errors.
    \item Evaluate routing accuracy, factual correctness, and response speed on Thai customer-service queries.
\end{enumerate}

\subsection{Scope}
The system covers FAQ, product information, and knowledge-base inquiries
in Thai, supporting four interaction modes: text-to-text, text-to-voice,
voice-to-text, and voice-to-voice.
Complex multi-turn conversations, emotion analysis, CRM integration, and
domain-specific model fine-tuning are outside the current
scope~\cite{manjuProgReport}.
...
The model was trained on $\sim$11{,}000 hours of Thai audio and evaluated on the Typhoon ASR Benchmark~\cite{sirichotedumrong2026typhoonasr}. Table~\ref{tab:typhoon_perf} summarises its key performance figures.

\begin{table}[h]
    \centering
    \caption{Typhoon ASR Real-Time key performance figures.}
    \label{tab:typhoon_perf}
    \small
    \begin{tabular}{@{}lp{4.5cm}@{}}
        \toprule
        \textbf{Metric} & \textbf{Value} \\
        \midrule
        Accuracy (CER) & 6.81\% vs.\ 5.84\% Pathumma ($13\times$ larger) \\
        Robustness & CER 10.36\%$\to$6.32\% on in-the-wild Thai media \\
        Throughput & 4{,}097$\times$ real-time (15--19$\times$ faster than Whisper) \\
        Cost & \$0.0023/hr on L4 GPU; up to 400$\times$ cheaper than cloud APIs \\
...
\subsubsection{From Design to Live Response}

Once a customer sends a query, the system reads the saved design and
follows it step by step.
Each component does its part---receiving the input, looking up relevant
facts, asking the AI to form an answer---and passes what it has found to
the next component in the chain.
This ensures the final reply always draws on all the information gathered
along the way.
The system also automatically detects whether the conversation is
text-based or voice-based and adjusts accordingly, so the operator does
not need to configure this separately.
