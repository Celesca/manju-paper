...
Trained on $\sim$11{,}000 hours of Thai audio, its key performance figures are shown in Table~\ref{tab:typhoon_perf}.
...
\noindent Its low latency, cost efficiency, and CPU compatibility make Typhoon ASR Real-Time the target server-side upgrade for MANJU (see Section~\ref{sec:conclusion}).
...
\begin{figure}[h]
    ...
    \caption{Demo Console showing a voice workflow test with real-time recording, chat transcript, and audio playback controls.}
    \label{fig:demo_console}
\end{figure}
